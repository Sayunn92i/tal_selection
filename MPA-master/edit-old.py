...
class Edit:
    ...
    """
    def move(self, event):
        #Dans le cas d'une selection rectangulaire
        if self.app.edit_selection_type.get() == "rect":
        ...
        #Dans le cas d'une selection elliptique
        elif self.app.edit_selection_type.get() == "oval":
            # Point mouvant de la zone d'édition
            x, y = self.app.canvas.canvasx(event.x), self.app.canvas.canvasy(event.y)
            self.edit_region[2] = x
            self.edit_region[3] = y
            ...
            min_latitude, min_longitude = tools.pixels_to_latlon(self.map.tiles_size, self.map.zoom_level, cx - rx, cy + ry)
            max_latitude, max_longitude = tools.pixels_to_latlon(self.map.tiles_size, self.map.zoom_level, cx + rx, cy - ry)
            self.app.min_latitude_value.config(text=min_latitude)
            self.app.min_longitude_value.config(text=min_longitude)
            self.app.max_latitude_value.config(text=max_latitude)
            self.app.max_longitude_value.config(text=max_longitude)
        #Dans le cas d'une selection avec la souris
        elif self.app.edit_selection_type.get() == "mouse":
            
            # Si le polygone n'a pas encore été créé, créer un polygone vide
            if not self.app.canvas.find_withtag("edit"):
                self.app.canvas.create_polygon(self.edit_region_mouse, fill="#3d9ccc",outline="#3588b2", stipple="gray25", tag="edit")
            else:
                # Ajouter les coordonnées de la nouvelle position de la souris à la liste de coordonnées du polygone
                self.edit_region_mouse.append(self.app.canvas.canvasx(event.x))
                self.edit_region_mouse.append(self.app.canvas.canvasy(event.y))
        ...
        if min_latitude and min_longitude and max_latitude and max_longitude:
            ...
            if self.app.edit_selection_type.get() == "rect":
                if not self.app.canvas.find_withtag("edit"):
                    ...
                    self.app.canvas.coords("edit", self.edit_region)
            if self.app.edit_selection_type.get() == "oval":
                if not self.app.canvas.find_withtag("edit"):
                    self.app.canvas.create_oval(self.edit_region, fill="#3d9ccc", outline="#3588b2", stipple="gray25", tag="edit")
                else:
                    self.app.canvas.coords("edit", self.edit_region)

            if self.app.edit_selection_type.get() == "mouse":
                if not self.app.canvas.find_withtag("edit"):
                    self.app.canvas.create_polygon(self.edit_region_mouse, fill="#3d9ccc", outline="#3588b2", stipple="gray25", tag="edit")
                else:
                    self.app.canvas.coords("edit", self.edit_region_mouse)
